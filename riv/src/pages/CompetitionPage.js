...
const useRivalProgress_1 = require("../hooks/useRivalProgress");
...
const CompetitionPage = () => {
    ...
    const { selectedCharacter, problemCount, timePerProblem, setUserProgress, rivalProgress, setRivalProgress, elapsedTime, setElapsedTime, isCompetitionActive, setIsCompetitionActive, setUserScore, setRivalScore, setRivalClearTime, } = (0, AppContext_1.useAppContext)();
    const [isFinished, setIsFinished] = (0, react_1.useState)(false);
    const [correctAnswers, setCorrectAnswers] = (0, react_1.useState)('');
    const [rivalFinished, setRivalFinished] = (0, react_1.useState)(false);
    const [rivalFinalScore, setRivalFinalScore] = (0, react_1.useState)(0);
    const [countdown, setCountdown] = (0, react_1.useState)(5);
    const [showCountdown, setShowCountdown] = (0, react_1.useState)(true);
    (0, react_1.useEffect)(() => {
    ...
    (0, useRivalProgress_1.useRivalProgress)({
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
        <div className="p-6 bg-gradient-to-r from-blue-50 to-purple-50 border-b">
          <div className="flex justify-between items-center">
            ...
            <Timer_1.default isRunning={isCompetitionActive && !isFinished} elapsedTime={elapsedTime} onTimeUpdate={setElapsedTime}/>
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <ProgressBar_1.default progress={isFinished ? 100 : 0} color="#3B82F6" label="Solving..."/>
            </div>
            
            <div>
              <div className="flex items-center mb-2">
                <img src={selectedCharacter.avatar} alt={selectedCharacter.name} className="w-8 h-8 rounded-full mr-2 object-cover"/>
                <h3 className="font-medium text-gray-700">{selectedCharacter.name}'s Progress</h3>
              </div>
              <ProgressBar_1.default progress={rivalProgress} color={selectedCharacter.color} label={`${Math.round(rivalProgress)}% Complete`}/>
            </div>
          </div>
        </div>
        
        <div className="p-6">
          {!isFinished ? (<div className="bg-gray-50 rounded-lg p-6 mb-6 border border-gray-100">
              ...
              <div className="flex justify-between">
                <button onClick={handleGiveUp} className="px-4 py-2 text-red-600 hover:text-red-700 font-medium flex items-center">
                  <lucide_react_1.Flag className="w-4 h-4 mr-1"/>
                  Give Up
                </button>
                
                <button onClick={() => setIsFinished(true)} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md flex items-center transition-colors">
                  Complete
                  <lucide_react_1.CheckCircle className="ml-2 w-4 h-4"/>
                </button>
              </div>
            </div>) : (<div className="bg-gray-50 rounded-lg p-6 mb-6 border border-gray-100">
              ...
              <div className="mb-6">
                ...
                <input type="number" min="0" max={problemCount} value={correctAnswers} onChange={handleCorrectAnswersChange} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder={`Enter a number between 0 and ${problemCount}`}/>
              </div>
              
              <button onClick={handleComplete} disabled={correctAnswers === ''} className={`w-full py-3 rounded-md font-medium text-white ${correctAnswers !== ''
                ? 'bg-blue-600 hover:bg-blue-700'
                : 'bg-gray-400 cursor-not-allowed'}`}>
                Submit Results
              </button>

              {rivalFinished && (<div className="mt-4 p-4 bg-yellow-50 border border-yellow-100 rounded-lg">
...
exports.default = CompetitionPage;
